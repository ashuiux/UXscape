  ...
  function ResearchMethodsPage() {
    ...
    const fetchMethods = useCallback(async () => {
      try {
        ...
        const parsedMethods = methodsData.map((method) => ({
          ...method,
        ...
        setFilteredMethods(parsedMethods);
      } catch (err) {
    ...
    const handleDeleteMethod = useCallback(async (deletedId) => {
      try {
        ...
        setMethods((prevMethods) =>
          prevMethods.filter((method) => method.id !== deletedId)
        );
        setFilteredMethods((prevMethods) =>
          prevMethods.filter((method) => method.id !== deletedId)
        );
      } catch (error) {
      ...
      try {
        ...
        setMethods((prevMethods) =>
          prevMethods.map((method) =>
            method.id === methodId ? editedMethod : method
          )
        );
        setFilteredMethods((prevMethods) =>
          prevMethods.map((method) =>
        ...
        <div className="research-methods__toggle">
          <label className="research-methods__switch">
            <input
              ...
              onChange={() =>
                handleFilterChange(
                  activeFilter === "new-design" ? "redesign" : "new-design"
                )
              }
